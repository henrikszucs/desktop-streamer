...
const ServerWS = class {
    ...
    // open the database and create the schema on the first boot
    async startDatabase(conf) {
        // Database connect
        const database = conf["ws"]["database"];
        let knexConf = null;
        if (database["type"] === "sqlite") {
            // a local file, the folder around it is ours to create
            try {
                await fs.mkdir(path.dirname(database["file"]), {"recursive": true});
            } catch (error) {
                throw new Error("Cannot create WS Database folder: " + path.dirname(database["file"]) + " - " + error.message);
            }
            knexConf = {
                ...
                // SQLite ignores the foreign keys of the schema without it
                "pool": {
            ...
            // mysql2 speaks the auth of both MySQL 8 and the older servers
            knexConf = {
                "client": "mysql2",
                "connection": {
                    "host": database["host"],
        ...
        try {
            this.db = knex(knexConf);
        } catch (error) {
            throw new Error("Cannot connect to WS Database with provided configuration: " + error.message);
        }

        // knex connects lazily, ask for the connection before the schema does
        try {
        ...
        if (await this.db.schema.hasTable("users") === false) {
            await this.db.schema.createTable("users", function (table) {
                ...
                table.string("email");      // indexed below, MySQL cannot index a TEXT without a key length
    ...
    async updateUserData(userId, type, data) {
        ...
        if (type === "picture") {
            ...
            const imageData = await httpsGetImage(data);
            value = imageData;
    ...
    async handleAPI(messageObj, clientId) {
        ...
        if (message["type"] === "user-data-subscribe") {
            ...
            if (key === "email") {
                ...
                const imageData = await httpsGetImage(userGoogle["picture"]);
                value = imageData;
